...
const StoriesPagination = ({ currentPage, totalPages, totalStories, perPage = 12 }) => {
  ...
  const handlePageChange = (page) => {
    if (page !== currentPage && page !== '...') {
      ...
      router.push(`/stories?${searchParams.toString()}`);
    }
  };
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4 mt-8">
      ...
      <div className="text-sm text-secondaryTextColor">
        نمایش {toFarsiNumber((currentPage - 1) * perPage + 1)} تا {toFarsiNumber(Math.min(currentPage * perPage, totalStories))} از {toFarsiNumber(totalStories)} داستان
      </div>
            ...
            {page === '...' ? (
              ...
              <button
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm transition-all ${
                  currentPage === page
                    ? 'bg-gradient-to-r from-primaryThemeColor to-primaryThemeColor text-white shadow-lg shadow-primaryThemeColor/30'
