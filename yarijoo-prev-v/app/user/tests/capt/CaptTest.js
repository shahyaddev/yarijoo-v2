...
const CaptTest = () => {
      ...
      .then((res) => {
        ...
        res.data.forEach((item) => {
          newStructure.push({
            ...item,
            an: [
              { key: 1, label: "کاملاً مخالفم", value: 1, default: false },
              { key: 2, label: "مخالفم", value: 2, default: false },
              {
                key: 3,
                label: "نه موافقم نه مخالفم",
                value: 3,
                default: false,
              },
              { key: 4, label: "موافقم", value: 4, default: false },
              { key: 5, label: "کاملاً موافقم", value: 5, default: false },
            ],
          });
        });
  ...
  const submitTestHandler = () => {
    console.log(answers);
    setLoading(true);
    postData("/tests/capt/answers-submit", { answers })
      .then((res) => {
        setLoading(false);
