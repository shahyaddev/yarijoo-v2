...
const GottmanTest = () => {
  ...
  useEffect(() => {
    getData("/tests/gottman/divorce/questions")
      .then((res) => {
        const newStructure = [];
        res.data.forEach((item) => {
          newStructure.push({
            ...item,
            an: [
              { key: 1, label: "بله", value: 1, default: false },
              { key: 2, label: "خیر", value: 0, default: false },
            ],
          });
        });
  ...
  const nextQuestionHandler = (id, data) => {
    ...
    // save answers
    setAnswers((prev) => [...prev, { question_id: id, answer: data }]);
  };
  ...
  const submitTestHandler = () => {
    ...
    const finallAnswer = answers.reduce((sum, item) => sum + item.answer, 0);
    const ones = answers.filter((item) => item.answer === 1).length;
    const zeros = answers.filter((item) => item.answer === 0).length;

    const percentOnes = (ones / total) * 100;
    const percentZeros = (zeros / total) * 100;

    setLoading(true);
    postData("/tests/gottman/divorce/answers-submit", {
      answers,
      total_answer: finallAnswer,
      one_percent: percentOnes,
      zero_percent: percentZeros,
    })
      .then((res) => {
        // setLoading(false);
                  ...
                  activeQs === i && (
                    ...
                    >
                      <p className="text-lg text-primaryTextColor font-bold">
                        <span>{toFarsiNumber(i + 1)}.</span> {question.question}
                      </p>
