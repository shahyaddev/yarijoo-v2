...
function formatAmount(value: number): string {
    ...
    if (value >= 1_000) return (value / 1_000).toFixed(0) + ' ک'
    return String(value)
}
...
export default function AdminReportsPage() {
    ...
    const fetchKpi = useCallback(async () => {
        try {
            const res = await api.get<KpiData>('/admin/dashboard')
            setKpi((res.data as { data?: KpiData }).data ?? res.data as unknown as KpiData)
        } catch { /* non-critical */ }
    }, [])
    ...
    const fetchReport = useCallback(async (type: ReportType) => {
        ...
        try {
            ...
            const raw = (res.data as { data?: { data: unknown[] } }).data ?? res.data as unknown as { data: unknown[] }
    ...
    const handleFilter = () => { void fetchReport(activeTab) }
    ...
    // Build monthly buckets from revenue data for chart
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
                ...
                <div className="flex items-end gap-4 flex-wrap">
                    ...
                    <button onClick={handleFilter} className="bg-primary-600 hover:bg-primary-700 text-white text-sm px-5 py-2.5 rounded-xl transition-colors">اعمال فیلتر</button>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'کل درآمد', value: kpi ? `${totalRevenue.toLocaleString('fa-IR')} ت` : '—', icon: '💰', color: 'text-green-400' },
                    { label: 'کل سفارشات', value: kpi ? revenueData.length.toLocaleString('fa-IR') : '—', icon: '🛒', color: 'text-blue-400' },
                    { label: 'کاربران کل', value: kpi ? kpi.total_users.toLocaleString('fa-IR') : '—', icon: '👤', color: 'text-purple-400' },
                    { label: 'اشتراک فعال', value: kpi ? kpi.active_subscriptions.toLocaleString('fa-IR') : '—', icon: '⭐', color: 'text-yellow-400' },
                ].map((item) => (
                ...
                <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
                    ...
                    <div className="flex items-end gap-3 h-48">
                        {chartEntries.map(([month, value]) => {
                            const heightPercent = (value / maxChartVal) * 100
                            return (
                                <div key={month} className="flex-1 flex flex-col items-center gap-2">
                                    <span className="text-xs text-gray-400">{formatAmount(value)}</span>
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
                <div className="flex border-b border-gray-800">
                    {([['revenue', '💰 درآمد'], ['users', '👥 کاربران'], ['tests', '🧠 تست‌ها']] as [ReportType, string][]).map(([tab, label]) => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                            className={['px-5 py-3 text-sm border-b-2 transition-colors', activeTab === tab ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-200'].join(' ')}>
                            {label}
                        </button>
                    ))}
                </div>
