...
export default function AdminDashboard() {
    ...
    const kpis = [
        { label: 'کل کاربران', value: stats?.totalUsers ?? 0, icon: '👥', color: '#E3F2FD', change: '' },
        { label: 'درآمد کل (ت)', value: stats?.totalRevenue ?? 0, icon: '💰', color: '#E8F5E9', change: '', money: true },
        { label: 'کل سفارشات', value: stats?.totalOrders ?? 0, icon: '🛍️', color: '#F3E5F5', change: '' },
        { label: 'تیکت باز', value: stats?.openTickets ?? 0, icon: '🎫', color: '#FFF8E1', change: '' },
    ]
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {kpis.map(k => (
                    <div key={k.label} className="rounded-xl border border-gray-800 p-4" style={{ background: '#111827' }}>
                        <div className="w-9 h-9 rounded-lg flex items-center justify-center text-lg mb-3" style={{ background: k.color }}>
                            {k.icon}
                        </div>
                        <div className="text-xl font-bold text-white mb-0.5">
                            {loading ? '…' : k.money
                                ? (k.value as number).toLocaleString('fa-IR')
                                : (k.value as number).toLocaleString('fa-IR')}
                        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    { href: '/admin/users', icon: '👥', label: 'مدیریت کاربران' },
                    { href: '/admin/shop', icon: '🛍️', label: 'محصولات' },
                    { href: '/admin/packages', icon: '📦', label: 'پکیج‌ها' },
                    { href: '/admin/sms-packages', icon: '📱', label: 'پکیج پیامکی' },
                    { href: '/admin/blog', icon: '📰', label: 'مقالات' },
                    { href: '/admin/tickets', icon: '🎫', label: 'تیکت‌ها' },
                    { href: '/admin/reports', icon: '📊', label: 'گزارش‌ها' },
